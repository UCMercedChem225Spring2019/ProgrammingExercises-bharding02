...
!
      ...
      allocate(sqFock(nBasis,nBasis),fockTilde(nBasis,nBasis),  &
        ...
        tempSqMatrix(nBasis,nBasis),SSPEV_Scratch(nBasis,3),sqOverlap&
                                                 (nBasis,nBasis))
!
!
...
!
!
      call SymmetricPacked2Matrix_UpperPac(nBasis,symCoreHamiltonian,sqCoreHamiltonian)
...
!     ^^^^This multiplies V (the E-vec matrix) with invsqrt(Lambda) (the
!     sqrt inverse of the e-val (diagonal) matrix) with V transpose.
!
...
!
!     This subroutine converts a matrix in upper/column storage form to
!     a rank-1 array.
!
!     Variable Declarations
!
